...
const FromUpload = () => {
  ...
  const handleSubmit = async(e) => {
    ...
    formData.append("file", showName);

    try {
      const response = await fetch(
        `https://sharelink-tau.vercel.app/api/v1/file/upload`,
        {
          method: "POST",
          body: formData,
          headers: {
  ...
  return (
    <div className="h-[100vh] bg-white flex flex-col items-center justify-center">
      <div className="w-max md:w-[500px] border border-black rounded-lg p-6 bg-white">
        <div className="space-y-1.5">
          <h5 className="whitespace-nowrap text-lg text-black font-medium tracking-tight ">
            Upload your file
          </h5>
          <p className="text-sm text-black">
            File Should be in PDF, DOCX or JPG formate
          </p>
        </div>
        {/* upload button  */}
        <div className="mt-10">
          <button className="w-full">
            <label
              className="flex h-full w-full  items-center justify-center gap-4 rounded-lg border  px-4 py-2 text-white active:ring-4 active:ring-cyan-200"
              htmlFor="file4"
            >
              <svg
                width={24}
              ...
              <p className="text-lg font-medium text-[#2E2E30] ">
                {" "}
          ...
          {showName.name ? null : (
            <p className="py-1 text-sm text-[#2E2E30]">No file Chosen</p>
          )}
        </div>
        {showName.name && (
          <div className="mt-10">
            <div className="flex items-center gap-4">
              ...
              >
                ...
                <g id="SVGRepo_iconCarrier">
                  ...
                  >
                    <g
                      id="Icon-Set"
                      transform="translate(-154.000000, -99.000000)"
                      fill="#000000"
                    >
                      <path
              ...
              <div>
                <p className="font-medium text-black">{showName?.name}</p>
      ...
      <div className="mt-10">
        <button
          ...
          className="text-xl w-32 h-14 bg-sky-500 text-white relative overflow-hidden group z-10 hover:text-white duration-1000"
        >
